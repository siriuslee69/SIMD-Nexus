...
- Unit tests for behavior and regressions in `evaluation/tests/`.
...
- `evaluation/tests/test_basic.nim`
...
5. Update `README.md` and `agents/PROGRESS.md` when behavior or workflow changes.

## Commands
- `nimble test`
- `nimble build`
- `nim c -r evaluation/tests/test_basic.nim`

## Progress and autopush
- Record the current commit message and work log in `agents/PROGRESS.md`.
- `nimble autopush` reads commit text from `agents/PROGRESS.md` (with compatibility fallbacks).

## Known environment caveat
Some environments fail to write Nimble metadata under `%USERPROFILE%\\.nimble` (for example `nimbledata2.json`). If that happens, run direct `nim c` commands to validate code paths while preserving the failure note in `agents/PROGRESS.md`.
